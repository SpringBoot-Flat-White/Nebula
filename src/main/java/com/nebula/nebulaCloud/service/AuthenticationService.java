...
/**
 * Service layer responsible for handling the business logic of user
 * authentication and registration.
 *
 * This service orchestrates the interaction between the user repository, the
 * password encoder,
 * the JWT service, and Spring's AuthenticationManager to provide secure auth
 * endpoints.
 */
@Slf4j
@Service
...
public class AuthenticationService {
         ...
         * <li>Creates and saves the new User and its associated Individual entity in a
         ...
         * @return A {@link ResponseEntity} with user details in the body and a
         *         'Set-Cookie' header containing the auth token.
         * @throws IllegalStateException if a user with the provided email already
         *                               exists.
         */
        @Transactional
        public ResponseEntity<AuthenticationResponse> register(RegisterRequest request) {
                ...
                // 4. Save the user. Due to CascadeType.ALL, the individual will be saved
                // automatically.
                userRepository.save(user);

                // 4.5. Send Discord notification asynchronously
                ...
                // 8. Return the response with the cookie in the header and user details in the
                // body
        ...
        /**
         * Authenticates an existing user based on their credentials and returns their
         * details.
         *
         * <p>
         * This method orchestrates the authentication process by performing the
         * following steps:
         * </p>
         * <ol>
         * <li>Delegates credential validation (email and password) to Spring Security's
         * {@link AuthenticationManager}.</li>
         * <li>If authentication is successful, it fetches the complete {@link User}
         * entity from the database.</li>
         * <li>Generates a JSON Web Token (JWT) for the authenticated user to manage
         * their session.</li>
         * <li>Embeds the generated JWT into a secure, <strong>HttpOnly</strong> cookie.
         * This cookie is automatically handled by the browser
         * and is not accessible to client-side scripts, mitigating the risk of XSS
         * attacks.</li>
         * <li>Constructs and returns a response body containing non-sensitive user
         * details (email, full name, user type).</li>
         * </ol>
         *
         * @param request The {@link AuthenticationRequest} DTO containing the user's
         *                email and password.
         * @return A {@link ResponseEntity} where the body contains an
         *         {@link AuthenticationResponse} with user details,
         *         and a 'Set-Cookie' header contains the JWT as a secure, HttpOnly
         *         cookie. The token itself is excluded from the response body.
         * @throws AuthenticationException if the credentials provided in the request
         *                                 are invalid.
         * @throws IllegalStateException   if the user is successfully authenticated but
         *                                 cannot be found in the repository,
         *                                 which indicates a potential data consistency
         *                                 issue.
         */
        ...
        public ResponseEntity<AuthenticationResponse> authenticate(AuthenticationRequest request) {
                ...
                // 7. Return the final ResponseEntity, adding the cookie to the headers and the
                // DTO to the body.
                return ResponseEntity.ok()
                                .header(HttpHeaders.SET_COOKIE, jwtCookie.toString())
                                .body(responseBody);
         ...
         *
         * <p>
         * This method creates a new {@link ResponseCookie} with the same name
         * ('access_token')
         * as the authentication cookie, but with a max age of 0. When sent to the
         * browser,
         * this instruction effectively overwrites and deletes the existing cookie,
         * terminating the user's session from the client's perspective.
         * </p>
         *
        ...
        public ResponseEntity<CompleteProfileResponse> completeProfile(String email, CompleteProfileRequest request) {
                ...
                // Send Discord notification for OAuth2 user completing profile
                discordNotificationService.sendUserRegistrationNotification(
                                user.getEmail(),
                                individual.getFullName(),
                                user.getUserType().toString());
